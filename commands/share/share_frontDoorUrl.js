...
(function () {
  ...
  module.exports = {
    topic: 'share',
    command: 'frontDoorURL',
    description: 'Provides a login URL to an org (scratch org or otherwise) using access token',
    help: 'Provides a login URL to an org (scratch org or otherwise) using access token',
    flags: [{
      name: 'alias',
      char: 'a',
    ...
    run(context){
      ...
      AccessUtil.getAccessInfo(context.alias)
        .then(function(accessInfo){
          const loginUrl = accessInfo.getFrontDoorLink(context.startUrl);
          if(context.json){
