...
class PackageListConverter {
   ...
   * Converts 
   * 
   * @param {MdApiPackage} mdApiPackage instance 
   * @param {string} targetPath - path to write the package.xml
   * @memberof PackageListConverter
   */
  convertMdApiPackageToPackageList(mdApiPackage, targetPath){
    {/* @TODO: complete **/}
  }
  ...
  convertPackageListToXml(sourcePath, targetPath, apiVersion){
    ...
    this.convertPackageListToJSON(sourcePath, apiVersion)
      .then(function(mdApiPackage){
        ...
        //console.log('mdApiPackage returned');
        //console.log(JSON.stringify(mdApiPackage, null, 2));

        return (PackageWriter.writeMdApiPackageToFile(mdApiPackage, targetPath));
      })
  ...
  /**
   * Converts an entire mdApiPackage package to the lines needed
   * for a packageList.
   * 
   * @param {MdApiPackage} mdApiPackage 
   * @returns {string[]} - collection of lines for a package list.
   * @memberof PackageListConverter
   */
  convertMdApiPackageToPackageListLines(mdApiPackage){
    if (!mdApiPackage){
      mdApiPackage = new MdApiPackage(MdApiPackage.DEFAULT_API_VERSION);
    }

    let self = this;

    const allPackageMembers = [];

    _.each(mdApiPackage.getTypes(), function(packageType){
      _.each(packageType.getMembers(), function(packageMember){
        allPackageMembers.push(packageMember);
      });
    });

    return this.convertMdApiMembersToPackageListLines(allPackageMembers);
  }
}
